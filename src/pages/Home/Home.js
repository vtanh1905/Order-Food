...
import { Layout, Col, Divider, Row, Icon, Card, Tabs, Drawer, Avatar, Input, Form, Button,
  Checkbox, List, Badge, notification  } from 'antd';
    ...
    constructor(props) {
        ...
        this.state = {
            foods: [
                ...
                {
                  id: 3,
                  src: 'http://www.savourydays.com/wp-content/uploads/2016/08/c%C3%A1ch-l%C3%A0m-b%C3%BAn-ch%E1%BA%A3-banner.jpg.jpg',
                  name: 'Bún chả',
                            ...
                            <List.Item>
                              ...
                              <List.Item.Meta
                                ...
                                <div style={{ width: '100%' }}>
                                  ...
                                  <Col span={20} offset={1}><h5 style={{ color: 'orange'}}>{item.title}</h5></Col></div> : 
                                <div>
                                  <Col span={3}><Icon type="coffee" /></Col>
                                  <Col span={20} offset={1}><h5 style={{ color: 'green'}}>{item.title}</h5></Col>
                                </div>}
                      ...
                      <Tabs activeKey={this.state.tab} onChange={this.handleTabChange}>
                          ...
                          el.type === 'MAIN' && <Col key={el.id} span={6} offset={(el.id + 2) % 3 === 0 ? 1 : 2 } style={{ display: 'flex', alignItems: 'center', flexDirection: 'column', marginBottom: 20, minWidth: '25%' }}>
                              <Card
                                  ...
                                  cover={<img alt={el.name} src={el.src} style={{ width: '100%', height: 200, border: '1px solid orange' }}/>}
                                  onClick={(id) => this.handleClickFoods(el.id)}
                              >
                              <Col span={3}><Icon type="star" /></Col>
                              <Col span={20} offset={1}><h5 style={{ color: 'orange'}}>{el.name}</h5></Col>
                              </Card>
                          </Col>
                          ))}
                          </TabPane>
                          ...
                          el.type === 'DESSERT' && <Col key={el.id} span={6} offset={(el.id + 2) % 3 === 0 ? 1 : 2 } style={{ display: 'flex', alignItems: 'center', flexDirection: 'column', marginBottom: 20, minWidth: '25%' }}>
                              <Card
                                  ...
                                  cover={<img alt={el.name} src={el.src} style={{ width: '100%', height: 200, border: '1px solid green' }}/>}
                                  onClick={(id) => this.handleClickFoods(el.id)}
                              >
                              <Col span={3}><Icon type="coffee" /></Col>
                              <Col span={20} offset={1}><h5 style={{ color: 'green'}}>{el.name}</h5></Col>
                              </Card>
                          </Col>
                          ))}
                          </TabPane>
                ...
                >
                  {visible && clickID !== -1 && <Layout style={{ background: 'white' }}>
                    <Form onSubmit={this.handleSubmit}>
                      <Row>
                        <Col span={4} style={{  }}><Avatar size={100} shape="square" src={foods[clickID -1].src}></Avatar></Col>
                        <Col span={19} ofsset={1} style={{ marginLeft: 20 }}>
                          <Row>
                            <h3>{foods[clickID - 1].name}</h3>
                          </Row>
                          <Row>
                            <p>{foods[clickID - 1].description}</p>
                          </Row>
                        </Col>
                      </Row>
